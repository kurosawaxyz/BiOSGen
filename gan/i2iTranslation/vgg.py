'''
Code used from: https://github.com/gordicaleksa/pytorch-neural-style-transfer/blob/master/models/definitions/vgg_nets.py
'''
...
class Vgg16(torch.nn.Module):
    ...
    def __init__(self, requires_grad=False):
        ...
        self.content_feature_maps_indices = [1]                                     # relu2_2
...
class Vgg19(torch.nn.Module):
    ...
    def __init__(self, requires_grad=False, use_relu=True):
        ...
        if use_relu:  # use relu or as in original paper conv layers
            ...
            self.offset = 0
        self.content_feature_maps_indices = [4]  # conv4_2
...
class Vgg16Experimental(torch.nn.Module):
    ...
    def __init__(self, requires_grad=False):
        ...
        self.content_feature_maps_indices = [4]
        self.style_feature_maps_indices = list(range(len(self.layer_names)))  # all layers used for style representation
        self.content_layer_names = [self.layer_names[x] for x in self.content_feature_maps_indices]
        self.style_layer_names = [self.layer_names[x] for x in self.style_feature_maps_indices]
